  ...
  <section class="space-y-8">
    ...
    <article class="surface-panel">
      ...
      <form class="grid gap-2 md:grid-cols-3" @submit.prevent="createCategory">
        <input v-model="categoryForm.name" class="field-input" placeholder="Category name" required />
        <input v-model="categoryForm.description" class="field-input" placeholder="Description" />
    ...
    <article class="surface-panel">
      ...
      <div class="space-y-2" v-if="store.categories.length">
        <div v-for="category in store.categories" :key="category._id" class="grid gap-2 rounded-xl border border-slate-200/90 bg-white/85 p-3 md:grid-cols-5">
          <input v-model="category.name" class="field-input md:col-span-2" />
          <input v-model="category.description" class="field-input md:col-span-2" />
          <div class="flex gap-2">
            <button class="rounded-lg bg-amber-500 px-3 py-1.5 text-sm font-semibold text-white" @click="updateCategory(category)">Save</button>
            <button class="rounded-lg bg-rose-600 px-3 py-1.5 text-sm font-semibold text-white" @click="removeCategory(category._id)">Delete</button>
          </div>
        </div>
      </div>
    </article>
    ...
    <article ref="createQuestionSection" class="surface-panel">
      ...
      <form class="grid gap-2 md:grid-cols-2" @submit.prevent="createQuestion">
        ...
        <button :disabled="!hasCategories" class="btn-primary disabled:cursor-not-allowed disabled:opacity-60 md:col-span-2">Add Question</button>
      </form>
    </article>
